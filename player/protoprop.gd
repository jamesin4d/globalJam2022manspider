...
const KEY_INSPECT: String = 'inspect'
 # an empty class ref to IITEM for inventory 
# Exported (Never update this on runtime)
export(float, 0.0, 90.0, 0.05) var snap_velocity = 25.0
export(float, 0.0, 90.0, 0.05) var let_go_distance = 0.8
export(float, 0.0, 1.0, 0.05) var mouse_x_sensitivity = 0.2
export(float, 0.0, 1.0, 0.05) var mouse_y_sensitivity = 0.2
...
func attach(actor: Node):
	...
	angular_damp = BUMP
	set_global_transform(prop_container.get_global_transform())
