...
export(float,-20,0.0,0.05) var webShoot_ray_length = -8
...
export(float, 0.0, 100.0, 0.05) var pull_jump_height = 10
...
export(float, 0.0, 10.0, 0.05) var hookin_speed = 4

# Runtime 
var extraVel := Vector3.ZERO # for hooking
...
var hookin:bool = false #is you hookin?
var hookinSpot
export var pause_focus: bool = false
# Nodes
onready var gui = get_node("GUI")
onready var hookpoint = get_node("HookPoint")
onready var debugger = get_node("GUI/HUD/DebugPanel")
onready var head = get_node('Head')
onready var db_label = get_node("GUI/HUD/DB")
...
onready var webRay = get_node("Head/CameraXPivot/WebCast")
onready var parent_state = get_node("../../../") #get the state this level scene is attached to 
#create_obj("Is debug build", funcref(OS, "is_debug_build"), null)

func _ready():
	webRay.cast_to.z = webShoot_ray_length
	var db = debugger
	db.add_child(db.create_obj("Web Crosshair collisions: ",funcref(self,"get_cross_data"),null))
	db.add_child(db.create_obj("velocity: ",funcref(self,"get_velocity_data"),null))
	debugger.refresh_objects_list()

func get_velocity_data():
	return str(velocity)

func get_cross_data():
	var objs = webRay.get_collider()
	return str(objs)
	
func resetHookin() -> void:
	hookin = false
		#in fucking bidness naawh cuntz. 
		
func cap_slutting_velocity():
	var topz = 25
	var topy = 15
	if velocity.x > topz:
		velocity.x = topz
	elif velocity.z > topz:
		velocity.z = topz
	if velocity.y > topy:
		velocity.y = topy
...
func toggle_pause_focus():
	pause_focus = !pause_focus
	if pause_focus:
			parent_state.emit_signal("mousemode","confine")
	if not pause_focus:
		parent_state.emit_signal("mousemode","capture")
...
func checkNHook() -> void:
	if webRay.is_colliding() and not hookin:
		hookin = true 
		hookinSpot = webRay.get_collider()
...
func _physics_process(delta):
	...
	#---------------------------------------------------------------------------
	if hookin:
		extraVel = normalize_prostitution()	
	# Combine
	#if Input.is_action_just_pressed("LMB") and webRay.is_colliding():
		#y_comp.y = 0 
	...
	velocity += extraVel
	#cap velocity while out on the streets slangin puss.
	cap_slutting_velocity()
	# Move the player
	var _velocity = move_and_slide(velocity, Vector3.UP, true, 4, FLOOR_MAX_ANGLE, false)
	

func normalize_prostitution():
	var force :Vector3= (hookinSpot.global_transform.origin-global_transform.origin)
	var exvel = Vector3.ZERO
	if force.length() < 20:
		exvel += (hookpoint.global_transform.origin-global_transform.origin).normalized() *2000/force.length()
	if force.length() < 10:
		exvel += (hookpoint.global_transform.origin-global_transform.origin).normalized()
	else:
		hookin = false
		resetHookin()
		force = Vector3.ZERO
